...
void ordenar(int v[], int n){
    ...
    int incrementos[] = {65536, 32767, 16383, 8191, 4095, 2047, 1023,
                         511, 255, 127, 63, 31, 15, 7, 3, 1};
    int valor, i, temp, j;

    for(valor = 0; valor <= 15; valor++){
        for(i = incrementos[valor]; i < n; i++){
            temp = v[i];
            j = i;
            while(j >= incrementos[valor] && v[j - incrementos[valor]] > temp){
                v[j] = v[j - incrementos[valor]];
                j = j - incrementos[valor];
            }
            v[j] = temp;
        }
    }
